...
module.exports = {
  ...
  async execute(bot, message, args) {
    ...
    if (!member) return message.channel.send("<:disagree:745003854017593354> You didn't provide a true user.");

    if (member.id === message.author.id) return message.channel.send("<:disagree:745003854017593354> You can't mute yourself.");
    if (member.roles.highest.position >= message.member.roles.highest.position && message.guild.owner.id != message.author.id) return message.channel.send("<:disagree:745003854017593354> You can't mute this member, they are too powerful for you.")
    if (!member.bannable) return message.channel.send("<:disagree:745003854017593354> This user is too powerful for me.");

    await Mute.findOne({ guildID: message.guild.id, userID: member.id }, async (err, mute) => {
      if (err) return message.channel.send(`An error occured: ${err}`);
      if (mute) return message.channel.send("<:disagree:745003854017593354> This user is already muted!");
      if (!mute) {
        ...
        await Guild.findOne({ guildID: message.guild.id }, async (err, guild) => {
          if (err) return message.channel.send(`An error occured: ${err}`);
          if (!guild) return message.channel.send("There was an error while fetching server database, please contact a bot dev! (https://discord.gg/J8Y2BB)");
          if (guild) {
        ...
        await muted.save().then(() => {
          member.roles.add(mutedrole).then(() => {
            if (!time) {
              message.channel.send(`Case: \`#${caseNumber}\` \n<:agree:745003896522670100> **${member.user.tag}** has been muted.`)
            } else {
              message.channel.send(`Case: \`#${caseNumber}\` \n<:agree:745003896522670100> **${member.user.tag}** has been muted for ${prettyms(time, {verbose: true})}.`)
            }
          }).catch(err => {
            message.channel.send(`An error occured: ${err}`)
          })
