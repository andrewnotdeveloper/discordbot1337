...
module.exports = {
  ...
  async execute(bot, message, args) {
    if (!args[0]) return message.channel.send("<:disagree:745003854017593354> You didn't provide a option or emoji.")

    if (args[0].toLowerCase() === "add" && message.member.hasPermission("MANAGE_EMOJIS")) {
      if (!args[1]) return message.channel.send("<:disagree:745003854017593354>  You didnt't provide a url or a emoji to add.");

      if (validURL(args[1])) {
        let name;
        if (args[2]) name = args[2]
        message.guild.emojis.create(args[1], name ? name : `emoji_${message.guild.emojis.cache.size}`).then(e => {
          return message.channel.send("<:agree:745003896522670100> Emoji has just been added to the server." + e.toString())
        }).catch(err => {
