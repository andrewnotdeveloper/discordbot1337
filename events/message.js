...
module.exports = {
  async execute(bot, message, db) {
                  ...
                  if (levelroles[0].lvl <= member.level) {
                    ...
                    if (!message.guild.members.cache.get(message.author.id).roles.cache.has(lvlrole.role)) {
                      ...
                      message.guild.members.cache.get(message.author.id).roles.add(lvlrole.role).then(() => {
                       ...
                       if (guild.settings.levelup.send) {
                         guild.settings.levelup.channel === "default" ? message.channel.send({embed: {description: levelupmessage }}) : bot.channels.cache.get(guild.settings.levelup.channel).send(`${message.author}`,{embed: {description: levelupmessage }});
                       }
                      }).catch(() => {
    ...
    return message.channel.send("<:disagree:745003854017593354> You are not accesed to use this command!")
    }
    if (command.guildOnly && message.channel.type === 'dm') {
      return message.channel.send('<:disagree:745003854017593354> This command cannot be executed in direct messages.');
    }
    if (command.reqPermissions) {
      ...
      if (missing.length > 0) {
        return message.channel.send("<:disagree:745003854017593354> You don't have the required permission(s) to use this command!! Missing permission(s): " + missing.join(', '));
      }
    }
